  ...
  <div class="p-4 md:p-8 w-full">
    ...
    <div v-else-if="filteredPlants.length === 0" class="text-center py-12">
      ...
      <p class="text-t-toned">
        {{ selectedFilter === 'reordered' ? 'Inga växter med omordnade bilder.' : 
           selectedFilter === 'not-reordered' ? 'Inga växter med icke-omordnade bilder.' :
    ...
    <div v-else class="">
      ...
      >
        ...
        <div class="flex items-start justify-between mb-4">
          <div>
            ...
            <div class="flex items-center gap-4 mt-2 text-sm text-t-muted max-md:flex-wrap">
              ...
              <UBadge
                :color="plant.images_reordered ? 'primary' : 'info'"
                variant="soft"
                size="sm"
              >
              ...
              <span class="flex gap-x-2 text-xs">
                  <span v-if="plant.images_reordered_date">
                    Uppdaterade: {{ new Date(plant.images_reordered_date).toLocaleDateString('sv-SE', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) }}
                  </span>
                  <span v-if="plant.images_added_date">
                    Från google: {{ new Date(plant.images_added_date).toLocaleDateString('sv-SE', { year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) }}
                  </span>
              </span>
            </div>
          </div>
        ...
        <!-- Images Grid -->
        <div v-if="plant.images && plant.images.length > 0" class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-10 gap-3">
          <div
          ...
          >
            ...
            <!-- Image Overlay Info -->
                <div class="image-overlay absolute inset-0 opacity-0 transition-opacity duration-300" >
                  <div class="w-full h-full p-2 text-white text-xs flex flex-col justify-between" @click="openImageViewer(plant.images, index)">
                    <!-- Actions -->
                    <div class="flex items-center justify-between gap-2 bg-black/70 rounded-md p-2" @click.stop>
                        <UButton 
                    ...
                    <div class="bg-black/60 rounded-md px-2 py-1 flex gap-1" @click.stop>
                        ...
                        <UTooltip :text="image.title || 'Utan titel'" :delay-duration="0">
                            <ULink :to="image.sourcePage" target="_blank" class="font-medium truncate text-white hover:text-white/70">{{ image.title || 'Utan titel' }}</ULink>
                        </UTooltip>
                    </div>
                  </div>
                </div>
    ...
    <UModal v-model:open="isAddImageModalOpen">
      <template #header>
        <h2 class="text-lg font-semibold">Lägg till bild{{ currentPlantForImage ? ` - ${currentPlantForImage.name}` : '' }}</h2>
      </template>
      <template #body>
          ...
          <div class="border-t border-border pt-6 space-y-4">
            ...
            <div class="flex gap-2">
              ...
              <div class="flex items-center gap-1 border border-border rounded-lg bg-bg">
                ...
                />
                <span class="px-3 py-1 text-sm font-semibold min-w-[3rem] text-center border-x border-border">{{ currentPage }}</span>
                <UButton
                  @click="goToPage(currentPage + 1)"
                  icon="i-heroicons-plus"
                  color="neutral"
            ...
            <div v-if="searchResults.length > 0" class="space-y-2">
              ...
              <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3 max-h-96 overflow-y-auto">
                ...
                >
                  ...
                  <div class="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity duration-200 flex items-center justify-center" @click="openSearchImageViewer(index)">
                    <UButton
                  ...
                  <!-- Image Title Tooltip -->
                  <div class="absolute bottom-0 left-0 right-0 bg-black/70 text-white text-xs p-2 opacity-0 group-hover:opacity-100 transition-opacity duration-200">
            ...
            <!-- No Results -->
            <div v-else-if="searchQuery && !isSearching && searchResults.length === 0 && searchHasBeenPerformed" class="text-center py-8 text-t-toned">
        ...
        <UModal v-model:open="isModalImageViewerOpen" fullscreen class="bg-black/90 z-50 flex items-center justify-center">
...
const { data: plants, error, pending, refresh } = await useAsyncData<Facit[]>(
  'plants-with-images',
  async () => {
    
    const { data, error } = await supabase
      .from('facit')
      .select('id, name, sv_name, images, images_reordered, images_reordered_date, images_added_date')
      .not('images', 'is', null)
      .order('name');
...
const filteredPlants = computed(() => {
  ...
  switch (selectedFilter.value) {
    case 'reordered':
      return plants.value.filter(plant => plant.images_reordered === true).sort((a, b) => {
        return new Date(b.images_reordered_date || '').getTime() - new Date(a.images_reordered_date || '').getTime();
      });
    case 'not-reordered':
      return plants.value.filter(plant => plant.images_reordered === false).sort((a, b) => {
        return new Date(b.images_added_date || '').getTime() - new Date(a.images_added_date || '').getTime();
      });
    default:
