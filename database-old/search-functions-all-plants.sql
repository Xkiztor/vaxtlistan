-- Search functions for PlantPicker component
-- These functions search ALL plants in the facit table (170k+ rows)
-- Used when adding new plants to lager where we need access to the complete plant database
...
-- Create optimized indexes for high-performance search on all plants
...
-- Function for ultra-fast prefix search on ALL plants
-- Used by PlantPicker for adding new plants to lager
CREATE OR REPLACE FUNCTION search_all_plants_prefix(
  search_term text,
...
-- Function for substring search on ALL plants
-- Used by PlantPicker for longer/more complex queries
CREATE OR REPLACE FUNCTION search_all_plants_substring(
  search_term text,
  result_limit integer DEFAULT 50,
  result_offset integer DEFAULT 0
)
...
-- Count function for ALL plants search results
CREATE OR REPLACE FUNCTION count_all_plants_search_results(
  search_term text,
...
-- Advanced trigram similarity search for ALL plants (use sparingly on free tier)
CREATE OR REPLACE FUNCTION search_all_plants_similarity(
  search_term text,
